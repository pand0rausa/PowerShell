...
echo `r`n | Out-File $results\test.txt -Append
    ...
    echo "**** c:\Windows\System32\sysprep\unattend.xml ****" | Out-File $results\test.txt -Append
    Select-String -SimpleMatch "==</Value>" -Path "C:\windows\System32\sysprep\unattend.xml" -Context 4,0 | Out-File $results\test.txt -Append
    echo `r`n | Out-File $results\test.txt -Append

    echo "**** c:\Windows\System32\sysprep\sysprepx64.xml ****" | Out-File $results\test.txt -Append
    Select-String -SimpleMatch "==</Value>" -Path "C:\windows\System32\sysprep\sysprepx64.xml" -Context 4,0 | Out-File $results\test.txt -Append
    echo `r`n | Out-File $results\test.txt -Append
    ...
    echo `r`n | Out-File $results\test.txt -Append
    reg query "HKCU\Software\pdfforge\PDFCreator\Settings\ConversionProfiles\0\EmailSmtp"  | Out-File $results\test.txt -Append
    echo `r`n | Out-File $results\test.txt -Append
    reg query "HKCU\Software\pdfforge\PDFCreator\Settings\ConversionProfiles\0\Ftp"  | Out-File $results\test.txt -Append
    echo `r`n | Out-File $results\test.txt -Append
    reg query "HKCU\SOFTWARE\Wow6432Node\Citrix\ICA Client\Engine\Lockdown Profiles\All Regions\Lockdown\Logon\Saved Credentials"  | Out-File $results\test.txt -Append
    echo `r`n | Out-File $results\test.txt -Append
    reg query "HKLM\SYSTEM\ControlSet\Control\Terminal Server\DefaultUserConfiguration"  | Out-File $results\test.txt -Append
    echo `r`n | Out-File $results\test.txt -Append
    reg query "HKLM\SYSTEM\ControlSet\Control\Terminal Server\WinStations\RDP-Tcp"  | Out-File $results\test.txt -Append
    echo `r`n | Out-File $results\test.txt -Append
